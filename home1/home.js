...
Page({
    ...
    getVideo(){
        let _this = this
        Request.request({
            url: `${oldServsers}/his-basic-data/api/videoPictureInformation/queryVideoPictureInformation`,
            method:'POST',
            data:{
                data:{
                    request:{
            ...
            success:res=>{
              console.log('Video:===>',res)
                _this.setData({
                    videoList:res.result.data.results
                })
            }
        })
    },
    ...
    goHotTopic(e){
        ...
        wx.navigateTo({
            url:'/pages/hotTopic/hotTopic?id='+id
        })
    },
    //去专病中心
    goSpecialCenter(){
        wx.navigateTo({
            url:'/pages/specialCenter/specialCenter'
        })
    },
    //点击导航 跳转
    goLocal(){
        ...
        //获取位置
        //注释的原因 每次获取位置的地方 不一定一样 可能位置换了
        // let location = wx.getStorageSync('indentLocation')
        ...
        wx.getLocation({
            ...
            success (res) {
                wx.navigateTo({
                    url:'/pages/selectLocal/selectLocal'
                })
                wx.setStorageSync('indentLocation',res)
            },
            fail(e){
                wx.navigateTo({
                    url:'/pages/selectLocal/selectLocal'
    ...
    //智能补方 地址
    Intellect(){
        let store = wx.getStorageSync('storeAndBusinessInfo')
        wx.navigateTo({
            url:'/pages/addDrugs/addDrugs?scene='+store.storeId
        })
    },
    //1.4以后首页点击"智能补方"继续跳转 问诊页面 首页涉及逻辑太多，新开一个吧function
    newImgQuick(){
    ...
    // 沿用之前的逻辑 跳转新的页面 只是跳转到新的页面
    newInquiryDoctor(){
        const { freeDoctor } = this.data;
        const { selectDocId, dialogType, interrogationType, doctorinfo } = this.data;
        ...
        Request.request({
            ...
            success: function ({ result }) {
                console.log('判断是否有该问诊方式的医生在线 准备跳转', selectDocId);
                ...
                if (freeDoctor || storeInfo.daojiaSpecialFlag == 0 || (doctorinfo && doctorinfo.doctorFlag === 1)) {
                    wx.navigateTo({
                        //产品让改过来，后续问题不能保证 也先改过来 直接跳转 咨询
                        url: `../selectPatient/selectPatient?doctorId=${selectDocId}&interrogationType=1&dialogType=${dialogType}`,
                    });
                } else {
    ...
    onShow: function (option) {
        //新需求 手动选中 地址 选中以后 回来 其他的不动 给个特许的情况
        if( getApp().globalData.newStoreId ){
            this.getStoreInfo(getApp().globalData.newStoreId);
        }
        // app.getUpdate();
        const that = this;
        ...
        /**
         * 新注册的会员，手机号没有更新到tokenInfo
         * 重新调一次登录接口
         */
        if(isMemberFlag && !tokenInfo.phone){
        login.wxLog()
        }
    ...
    inquiryDoctor(type) {
        ...
        Request.request({
            ...
            success: function ({ result }) {
                ...
                if (freeDoctor || storeInfo.daojiaSpecialFlag == 0 || (doctorinfo && doctorinfo.doctorFlag === 1)) {
                    wx.navigateTo({
                        url: `../addDrugs/addDrugs?doctorId=${selectDocId}&interrogationType=${interrogationType}&dialogType=${dialogType}`,
                    });
                    // wx.navigateTo({
    ...
    handleRequestSubscribe() {
        ...
        let self = this;
        const {zyMedicationSubscribe={}} = app.globalData;
        const {
            tmplIds = [],
            templateIdShort = '',
            bizCode
        } = zyMedicationSubscribe; // 根据环境切换全局配置的左医的订阅提醒
        const tokenInfo = wx.getStorageSync('tokenInfo') || {}

        // 是否成功订阅了用药提醒
        const reminderSubscribe = wx.getStorageSync('medicationSubscribe') || false
        const isMemberFlag = wx.getStorageSync('isMemberFlag') || false

        console.log('HandleRequestSubscribe:====>>>')
        // 如果不是会员的话，就弹窗获取手机号
        if(!isMemberFlag){
            this.goToRegister();
            return
        }
        // 如果已经订阅成功了,并且已经授权获取手机号注册成为会员了，才能跳转到用药提醒页面
        if(reminderSubscribe && isMemberFlag){
            this.goToMedication()
            return
        }
        ...
        wx.requestSubscribeMessage({
            ...
            success: res => {
                if (res[tmplIds[0]] === "accept") {
                    Request.request({
                        url: oldServsers + '/wxgateway/api/insertUserSubRecords',
                        method: 'POST',
                        data: [
                        {
                        ...
                        success: () => {
                        // 用药提醒订阅接口成功了，将这个状态
                        wx.setStorageSync('medicationSubscribe', true)
                        self.goToMedication()
                        },
                        noSuccess: res => {
